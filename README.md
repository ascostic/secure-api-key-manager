...
1.A user logs in and receives a JWT access token.
2.Using that token, the user can create an API key.
3.The API key is generated securely and hashed before being stored.
4.To access protected data, the client must send the API key in   the request header.
5.The system verifies the key and grants or denies access.
